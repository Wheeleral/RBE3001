...
arm.setPositionInverse(vertex(3,1),vertex(3,2),vertex(3,3));
pause(.5);
...
while 1
    ...
    for i = 1:3 %for each vertex
        ...
        %while(not(arm.atSetpoint(vertex(i,1),vertex(i,2),vertex(i,3))))
            ...
            vals=trajectoryparams(0,1, vertex(prev,1),vertex(i,1),vertex(prev,2), vertex(i,2), vertex(prev,3), vertex(i,3), spacing, 0, 0);
...
%                  historyX = [historyX curXYZ(1)];      %<-data collection
%               
...
%      historyY = [historyY curXYZ(2)];
%      historyZ = [historyZ curXYZ(3)];
      plotDataPos(totalvalsx,totalvalsy,totalvalsz); 
